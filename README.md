...
## The Project
In this project the attitude and IMU (accelerometer and gyroscope) biases are estimated using a set of 3 non-linear observers, the Global Observer (GO), the Biased Vector Observer (BVO) and the Partial Attitude Estimator (PAE).
Each observer has conditions under which it can be used and provably converges to the truth and what each estimates. 
This combination allows for attitude and gyroscope bias estimation despite aggresive maneuvers, and even allows for some estimation of accelerometer biases. 
...
Eventually, these labels will be used to train a neural network to decide online in real-time which observer to use, however this is future work.

## What I tried
At first I tried to use Q-learning to directly learn when each should be used.
The reward function was straight forward since low estimation error is obviously the main desire, but even trying to overfit one trajectory was taking over an hour, so trying to generalize looked like would take much longer than we had.
...
A depth of 7 seemed to result in the best performance tradeoff wise.
